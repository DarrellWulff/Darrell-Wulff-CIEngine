class TexturedRefractiveMaterial extends TexturedMaterial
{
	constructor(program, imageIDs, reflectionIntensity = 0.1, refractionIntensity = 0.9, refractiveIndex = 2.0, diffuse=1.0, specular=1.0, ambient=1.0, shininess=1.0)
	{
		super(program, diffuse, specular, ambient, shininess);
		...
		this.cubemapTextureLocation = this.gl.getUniformLocation(
			this.program, 'cubemap'
		);
	...
	deactivate()
	{
		...
		this.gl.activeTexutre(this.gl.TEXTURE1);
		this.gl.bindTexture(this.gl.TEXTURE_CUBE_MAP, null);
	}
}