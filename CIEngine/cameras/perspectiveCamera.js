...
{
	constructor(aspect=1, viewRadians=Math.PI/4, near=0.01, far=1000.0, position=new Vector(), rotation=new Quaternion())
	{
		super(position, rotation);

		// WebGL references / locations
		this.gl = Renderer.gl;
		this.programs = Renderer.shaderList;
		this.positionAttribLocations = [];
		this.mProjViewAttribLocations = [];

		for (let i = 0; i < Renderer.shaderList.length; i++)
		{
	...
	updateProjectionMatrix()
	{
		...
		if (this.hasSkybox)
		{
			this.skybox.updateProjectionMatrix();
		}
	}
